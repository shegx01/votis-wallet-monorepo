defmodule BeVotisWalletWeb.LoginController.Response do
  ...
  def create(conn, params) do
    with {:ok, validated_params} <- validate_and_parse_params(params),
      ...
      {:error, :organization_not_found} ->
        ...
        |> json(%{error: "Organization not found", message: "The specified organization does not exist"})
      ...
      {:error, :authentication_failed, status_code, error_message} ->
        ...
        |> json(%{error: "Authentication failed", message: "Invalid credentials or authentication service error"})
        
      {:error, reason} ->
        Logger.error("Unexpected error during login", error: inspect(reason))
  ...
  defp validate_organization_exists(org_id) do
    ...
    decoded_org_id = 
      case Base.decode64(org_id) do
        {:ok, decoded} -> decoded
        :error -> org_id  # If not base64 encoded, use as-is
      end
  ...
  defp perform_authentication(%LoginParams{auth_type: "passkey"} = params) do
    ...
    case Activities.client_signed_request(params.stamped_body, params.stamp, auth_type: :passkey) do
      ...
      {:error, status_code, error_message} -> {:error, :authentication_failed, status_code, error_message}
    end
  end
  ...
  defp extract_jwt_from_session(session_data) do
    ...
    case session_data do
      ...
      %{"activity" => %{"result" => %{"activity" => %{"result" => %{"oauthLoginResult" => %{"session" => session_token}}}}}} ->
        session_token
      
      # Stamp/Passkey login response - nested structure
      %{"activity" => %{"result" => %{"activity" => %{"result" => %{"stampLoginResult" => %{"session" => session_token}}}}}} ->
        session_token
  ...
  defp find_session_in_nested_result(result) do
    cond do
      ...
      is_map(result["activity"]) and is_map(result["activity"]["result"]) ->
        ...
        cond do
          Map.has_key?(nested_result, "oauthLoginResult") and Map.has_key?(nested_result["oauthLoginResult"], "session") ->
            nested_result["oauthLoginResult"]["session"]
          Map.has_key?(nested_result, "stampLoginResult") and Map.has_key?(nested_result["stampLoginResult"], "session") ->
