defmodule BeVotisWalletWeb.SignRawPayloadController.Response do
  ...
  def create(conn, params) do
    case conn.assigns[:user] do
      %User{} = user ->
        ...
        handle_sign_raw_payload(conn, params, user)

      nil ->
        # User doesn't exist, return 404
        Logger.warning("Sign raw payload attempt for non-existent user",
  ...
  # Private function to handle raw payload signing flow
  defp handle_sign_raw_payload(conn, params, user) do
    with {:ok, stamped_body} <- extract_stamped_body(params),
         {:ok, stamp} <- extract_stamp(params),
         {:ok, turnkey_response} <- sign_turnkey_raw_payload(stamped_body, stamp) do
      # Log successful raw payload signing
      Logger.info("Successful raw payload signing",
        email: user.email,
        user_id: user.id,
      ...
      {:error, :missing_parameter, param} ->
        Logger.warning("Missing required parameter for raw payload signing",
          parameter: param,
          user_id: user.id
        )
      ...
      {:error, :turnkey_error, status_code, error_message} ->
        Logger.error("Turnkey API error during raw payload signing",
  ...
  defp sign_turnkey_raw_payload(stamped_body, stamp) do
