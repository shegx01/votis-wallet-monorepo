defmodule BeVotisWallet.Services.Turnkey.SessionsTest do
  ...
  describe "create_read_write_session_for_client/4" do
    test "creates session and returns encrypted credential bundle for client" do
      ...
      stub(Mock, :build_payload, fn method, url, headers, body ->
        ...
        assert [{"Content-Type", "application/json"}, {"X-Turnkey-API-Key", "test_api_key"}] = headers
      ...
      # Test the function
      assert {:ok, session} = Sessions.create_read_write_session_for_client(
        organization_id,
        client_public_key,
        user_id,
    ...
    test "handles activity creation failure" do
      ...
      assert {:error, {403, ^error_message}} = Sessions.create_read_write_session_for_client(
        organization_id,
        client_public_key,
        nil,
        []
  ...
  describe "create_read_write_session_for_server/3" do
    ...
    test "accepts required options and creates server session with credential decryption" do
      ...
      result = Sessions.create_read_write_session_for_server(
        organization_id,
        nil,
  ...
  describe "create_read_only_session/3" do
    test "creates read-only session successfully" do
      ...
      expected_session_result = expected_response["activity"]["result"]["createReadOnlySessionResult"]
    ...
    test "handles read-only session creation failure" do
      ...
      assert {:error, {400, ^error_message}} = Sessions.create_read_only_session(organization_id, user_id)
    end
  end
  ...
  describe "integration workflow keypair validation" do
    ...
    test "validates server vs client session workflow differences" do
      ...
      client_public_key = public_hex  # Client would generate this themselves
      ...
      refute Keyword.has_key?(client_opts, :hpke_private_key)  # Client keeps this secret
    end
  end
end
