defmodule BeVotisWalletWeb.SignRawPayloadsControllerTest do
  ...
  setup %{user_attrs: user_attrs} do
    ...
    users = %{
      multisigner: insert(:user, user_attrs.multisigner),
      singlemulti: insert(:user, user_attrs.singlemulti),
  ...
  describe "POST /private/sign_raw_payloads" do
    test "successfully signs multiple raw payloads for existing user", context do
      %{conn: conn, users: users, turnkey_responses: responses, request_params: params} = context

      setup_detailed_turnkey_mock(responses.success_multiple)

      conn = post_sign_raw_payloads(conn, users.multisigner, params.valid)
    ...
    test "handles single payload in array format", context do
      %{conn: conn, users: users, turnkey_responses: responses, request_params: params} = context

      setup_simple_turnkey_mock(responses.success_single)

      conn = post_sign_raw_payloads(conn, users.singlemulti, params.single)
      response = json_response(conn, 200)
    ...
    test "handles non-standard Turnkey response structure gracefully", context do
      ...
      conn = post_sign_raw_payloads(conn, users.custom, params.custom)
      response = json_response(conn, 200)
    ...
    test "handles missing result in response with fallback message", context do
      ...
      conn = post_sign_raw_payloads(conn, users.no_result, params.basic)
      response = json_response(conn, 200)
    ...
    test "returns 400 for missing required parameters", context do
      ...
      for %{params: params, expected_field: field} <- test_cases do
        conn = post(conn, ~p"/private/sign_raw_payloads", params)
    ...
    test "returns 404 for non-existent user", context do
      ...
      conn =
        post(conn, ~p"/private/sign_raw_payloads", params.nonexistent_user)
    ...
    test "returns 401 for Turnkey authentication failure", context do
      ...
      conn = post_sign_raw_payloads(conn, users.auth_fail, params.invalid_auth)
    ...
    test "returns 400 for Turnkey bad request", context do
      ...
      setup_failed_turnkey_mock(400, %{"message" => "Malformed payloads data"})

      conn = post_sign_raw_payloads(conn, users.bad_request, params.malformed)
      response = json_response(conn, 400)
    ...
    test "returns 422 for Turnkey unprocessable entity", context do
      ...
      setup_failed_turnkey_mock(422, %{"message" => "Invalid payloads format"})

      conn = post_sign_raw_payloads(conn, users.unprocessable, params.invalid_format)
    ...
    test "returns 500 for unknown Turnkey error codes", context do
      ...
      conn = post_sign_raw_payloads(conn, users.unknown_error, params.basic)
  ...
  defp build_test_user_attrs do
    %{
      multisigner: %{email: "multisigner@example.com", sub_org_id: "test_org_123"},
      singlemulti: %{email: "singlemulti@example.com", sub_org_id: "test_org_456"},
  ...
  defp post_sign_raw_payloads(conn, user, params) do
    post(conn, ~p"/private/sign_raw_payloads", %{
      "email" => user.email,
