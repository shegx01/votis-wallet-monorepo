defmodule BeVotisWalletWeb.SignRawPayloadControllerTest do
  ...
  describe "POST /private/sign_raw_payload" do
    test "successfully signs raw payload for existing user", %{conn: conn} do
      # Create a user in the database
      user = insert(:user, %{email: "rawsigner@example.com", sub_org_id: "test_org_123"})

      # Mock successful raw payload signing response with r,s,v components
      signing_response = %{
        "activity" => %{
    ...
    test "returns 400 for missing required parameters", %{conn: base_conn} do
      ...
      for %{params: params, expected_field: field} <- test_cases do
        conn = post(base_conn, ~p"/private/sign_raw_payload", params)
    ...
    test "returns 400 for Turnkey bad request", %{conn: conn} do
      ...
      stub(Mock, :request, fn _payload ->
        {:error, 400, %{"message" => "Malformed payload data"}}
      end)
    ...
    test "returns 422 for Turnkey unprocessable entity", %{conn: conn} do
      ...
      # Mock Turnkey unprocessable entity (invalid payload format)
      stub(Mock, :build_payload, fn _method, _url, _headers, _body ->
        %{method: :post, url: "test", headers: [], body: ""}
      end)

      stub(Mock, :request, fn _payload ->
        {:error, 422, %{"message" => "Invalid payload format"}}
      end)
