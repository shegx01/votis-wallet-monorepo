...
class IOSGoogleAuthClient : GoogleAuthClient {
        ...
        try {
            ...
            suspendCancellableCoroutine { continuation ->
                ...
                GIDSignIn.sharedInstance.signInWithPresentingViewController(
                    ...
                    when {
                        error != null -> {
                            val authResult = if (error.code == -5L) { // User cancelled
    ...
    override suspend fun getCurrentUser(): AuthUser? = withContext(Dispatchers.Main) {
        GIDSignIn.sharedInstance.currentUser?.toAuthUser()
    }

    override suspend fun isSignedIn(): Boolean = withContext(Dispatchers.Main) {
